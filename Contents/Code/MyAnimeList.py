...
def GetMetadata(myanimelistIds, media_type, dict_AniDB):
  ...
  mal_cache = {}  # {malId: (parsed_response, score)} - avoid re-fetching same IDs across seasons
  for season in sorted(Dict(myanimelistIds, 'seasons'), key=natural_sort_key):
    ...
    for myanimelistId in sorted(season_mal_id_list, key=natural_sort_key):
      ### Use cached result if this MAL ID was already fetched in a previous season
      if myanimelistId in mal_cache:
        parsed_response, current_score = mal_cache[myanimelistId]
        Log.Info("=== MAL ID: {} (cached) ===".ljust(157, '=').format(myanimelistId))
      ...
      except Exception as e:  Log.Error("No Detail Information were available " + str(e));  mal_cache[myanimelistId] = ({}, 0);  continue
      ### Parse json data
      if json:
        ...
        mal_cache[myanimelistId] = (parsed_response, current_score)
        if current_score > best_score:
          best_score = current_score
          best_match = parsed_response
